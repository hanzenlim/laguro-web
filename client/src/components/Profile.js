...
class Profile extends Component {
    ...
    async getDentist() {
        ...
        await getDentistByUser(auth.id);
    ...
    getSortedListings(office) {
        ...
        if (listings && listings.length) {
            ...
            filteredListings = filteredListings.sort((listing_a, listing_b) => moment(listing_a.time_available).isAfter(moment(listing_b.time_available)));
    ...
    deleteListing(listing) {
        ...
        // eslint-disable-next-line
      confirm(
                `Delete listing for ${moment(listing.time_available).format('MMM D, h a')}?`)
        ) {
    ...
    renderActions() {
        const { dentist } = this.props;
        const dentistProfileExists = dentist && Object.keys(dentist).length !== 0;
  ...
  deleteReservation(reservation) {
      ...
      // eslint-disable-next-line
      confirm(
              `Delete reservation for ${moment(reservation.time_start).format('MMM D, h a')}?`)
      ) {
