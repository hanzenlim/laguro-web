...
class ReservationOptions extends Component {
    ...
    createReservation(values) {
        const {
            listing, office, time_start, time_end,
        } = this.props;
        ...
        if (
            ...
            const equip_selected = values.equip_selected.filter(equip => equip.needed);

            const staff_selected = values.staff_selected.filter(staff => staff.count > 0);
  ...
  renderStaff = ({ fields, className }) => {
      ...
      const staffData = this.props.staff_selected;
      if (!staffData) {
  ...
  renderEquipment = ({ fields, className }) => {
      const equipData = this.props.equip_selected;
      return (
  ...
  calcTime() {
      const { time_start, time_end } = this.props;
      const minutes = time_end.diff(time_start, 'minutes');
      this.hours = minutes / 60;
  }

  calcBookingFee() {
      const { chairs_selected, listing } = this.props;
      const chair_price = chairs_selected * listing.price * this.hours;
