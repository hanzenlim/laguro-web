...
class EditDentist extends Component {
    ...
    onSubmit(values) {
        ...
        this.props.editDentist({ ...values, imgUrl, id: auth._id });
        reset();
    }
    ...
    async getDentist() {
        ...
        if (dentists.length) {
            const myDentist = dentists.filter(dentist => dentist.user_id === auth._id);
            return myDentist[0];
        }
        return {};
    }
    ...
    extractUrlToState(result) {
        ...
        if (upload.length) {
            this.setState({ imgUrl: upload[0].url });
        }
    }

    renderUploadedImages() {
        const { imgUrl } = this.state;
        return <img src={imgUrl} alt="dentist" />;
    }
