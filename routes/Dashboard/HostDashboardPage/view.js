...
import { getKeyFromText } from '~/routes/Dashboard/Dashboard/utils';
import { addSearchParams, redirect } from '~/util/history';
import {
    DashboardGrid,
    StyledDashboardMenu,
    StyledDashboardMenuItem,
} from '~/routes/Dashboard/Dashboard/common';
import HostListing from '~/common/HostListings';
...
class HostDashboardPageView extends Component {
    handleClick = ({ key }) => {
        ...
        if (key === ADD_A_NEW_OFFICE_MENU_TEXT) {
            window.location.href = `${HOST_ONBOARDING_PAGE_URL_PREFIX}/add-office`;
            return;
        }
        ...
        history.push(`/host?${queryString.stringify(newParams)}`);
    };
