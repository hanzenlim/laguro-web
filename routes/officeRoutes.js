...
module.exports = app => {
  ...
  app.post("/api/offices", async (req, res) => {
    const { name, location, chairs, equipment, img_url } = req.body;
    ...
    let dentistsOffices = await Office.find({ user: req.user._id });
    res.send(dentistsOffices);
  });

  //edit office route
  app.patch("/api/offices", async (req, res) => {
    const { name, img_url, chairs, equipment, location, id } = req.body;
